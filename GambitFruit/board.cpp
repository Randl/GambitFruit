...
bool board_is_ok(const board_t *board) {
	...
	for (U16 sq = 0; sq < SquareNb; sq++) {
		...
		S32 piece = board->square[sq], pos = board->pos[sq];
	...
	for (S8 colour = 0; colour < ColourNb; colour++) {
		...
		S32 sq = board->piece[colour][size];
	...
	// material
	if (board->piece_nb != board->piece[White].size() + board->pawn[White].size() + board->piece[Black].size()
		+ board->pawn[Black].size()) {
		return false;
	}
...
void board_init_list(board_t *board) {
	...
	for (S8 colour = 0; colour < ColourNb; colour++) {
		...
		if (DEBUG) {
			...
			for (U16 i = 0; i < board->piece[colour].size(); i++) {
				S32 sq = board->piece[colour][i];
