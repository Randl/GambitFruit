...
static constexpr S8 ShortSearchDepth = 1;
...
static constexpr S16 EasyThreshold = 150;
static constexpr double EasyRatio = 0.20;

static constexpr double EarlyRatio = 0.60;

static constexpr S16 BadThreshold = 50; // 50
...
bool depth_is_ok(S32 depth) {
	return depth > -128 && depth < DepthMax;
}

// height_is_ok()

bool height_is_ok(S32 height) {
...
void search() {
	...
	if (option_get_bool("OwnBook") && !SearchInput->infinite) {
		const U16 move = book_move(SearchInput->board);
		if (move != MoveNone) {
			// play book move
	...
	*SearchRoot->list = SearchInput->list;
	...
	for (S32 depth = 1; depth < DepthMax; ++depth) {
		for (SearchCurrent->multipv = 0; SearchCurrent->multipv <= SearchInput->multipv; SearchCurrent->multipv++) {
			...
			if (SearchInput->time_is_limited && SearchCurrent->time * 2 >= SearchInput->time_limit_1
				&& !SearchRoot->bad_2)
				SearchRoot->flag = true;

			if (SearchInput->time_is_limited && SearchCurrent->time >= SearchInput->time_limit_1 * EasyRatio
				&& SearchRoot->easy) {
			...
			if (SearchInput->time_is_limited && SearchCurrent->time >= SearchInput->time_limit_1 * EarlyRatio
				&& !SearchRoot->bad_2 && !SearchRoot->change)
				SearchRoot->flag = true;

			if (SearchInfo->can_stop && (SearchInfo->stop || (SearchRoot->flag && !SearchInput->infinite))) {
				search_ready = true;
				break;
			}
		}
...
void search_update_best() {
	...
	if (DispBest) {
		...
		U16 move = SearchBest[SearchCurrent->multipv].move;
		S32 value = SearchBest[SearchCurrent->multipv].value, flags = SearchBest[SearchCurrent->multipv].flags,
			depth = SearchBest[SearchCurrent->multipv].depth;
		const mv_t *pv = SearchBest[SearchCurrent->multipv].pv;

		const S32 max_depth = SearchCurrent->max_depth;
		...
		if (SearchCurrent->multipv == 0) {
			...
			if (found) {
				for (S32 z = SearchCurrent->multipv; z > i; --z) {
		...
		if (depth > 1 || (depth == 1 && SearchCurrent->multipv == SearchInput->multipv)) {
			for (S32 i = 0; i <= SearchInput->multipv; ++i) {
				if (save_multipv[i].mate == 0) {
					...
					if (false) {
						...
						send("info multipv %d depth %d seldepth %d score cp %d time %.0f nodes " S64_FORMAT " pv %s",
						     ...
						     save_multipv[i].time,
						     save_multipv[i].node_nb,
						     save_multipv[i].pv_string);
					} else if (flags == SearchLower) {
						send("info multipv %d depth %d seldepth %d score cp %d lowerbound time %.0f nodes " S64_FORMAT " pv %s",
						     i + 1,
						     save_multipv[i].depth,
						     save_multipv[i].max_depth,
						     save_multipv[i].value,
						...
						send("info multipv %d depth %d seldepth %d score cp %d upperbound time %.0f nodes " S64_FORMAT " pv %s",
						     i + 1,
						     save_multipv[i].depth,
						     save_multipv[i].max_depth,
						     save_multipv[i].value,
					...
					if (false) {
						...
						send("info multipv %d depth %d seldepth %d score mate %d lowerbound time %.0f nodes " S64_FORMAT " pv %s",
						     i + 1,
						     save_multipv[i].depth,
						     save_multipv[i].max_depth,
						     save_multipv[i].mate,
						...
						send("info multipv %d depth %d seldepth %d score mate %d upperbound time %.0f nodes " S64_FORMAT " pv %s",
						     i + 1,
						     save_multipv[i].depth,
						     save_multipv[i].max_depth,
						     save_multipv[i].mate,
...
void search_update_current() {
	...
	my_timer_t *timer = SearchCurrent->timer;
	S64 node_nb = SearchCurrent->node_nb;
	const double time = (UseCpuTime) ? my_timer_elapsed_cpu(timer) : my_timer_elapsed_real(timer),
		speed = (time >= 1.0) ? double(node_nb) / time : 0.0, cpu = my_timer_cpu_usage(timer);
...
void search_check() {
	...
	if (SearchInput->depth_is_limited && SearchRoot->depth > SearchInput->depth_limit)
		SearchRoot->flag = true;

	if (SearchInput->time_is_limited && SearchCurrent->time >= SearchInput->time_limit_2)
		SearchRoot->flag = true;

	if (SearchInput->time_is_limited && SearchCurrent->time >= SearchInput->time_limit_1 && !SearchRoot->bad_1
		&& !SearchRoot->bad_2 && (!UseExtension || SearchRoot->move_pos == 0))
		SearchRoot->flag = true;
