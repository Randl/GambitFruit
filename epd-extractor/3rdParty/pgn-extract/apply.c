...
static Boolean
	play_moves(Game *game_details, Board *board, Move *moves, unsigned max_depth, Boolean check_move_validity);
static Boolean
	apply_variations(const Game *game_details, const Board *board, Variation *variation, Boolean check_move_validity);
 ...
 */
static char SAN_piece_characters[NUM_PIECE_VALUES] = {'?', '?', 'P', 'N', 'B', 'R', 'Q', 'K'};
 ...
 */
static const char *output_piece_characters[NUM_PIECE_VALUES] = {"?", "?", "P", "N", "B", "R", "Q", "K"};

/* letters should contain a string of the form: "PNBRQK" */
void set_output_piece_characters(const char *letters) {
	if (letters == NULL) {
		fprintf(GlobalState.logfile, "NULL string passed to set_output_piece_characters.\n");
	} else {
		Piece piece;
		int piece_index;
		for (piece_index = 0, piece = PAWN; piece <= KING && letters[piece_index] != '\0'; piece++) {
			...
			if (letters[piece_index + 1] == '+') {
				...
				if (letters[piece_index] != '\0') {
					...
					fprintf(GlobalState.logfile, "Missing piece letter following + in -Wsan%s.\n", letters);
					exit(1);
				}
			} else {
		...
		if (piece < NUM_PIECE_VALUES) {
			fprintf(GlobalState.logfile, "Insufficient piece letters found with -Wsan%s.\n", letters);
			fprintf(GlobalState.logfile, "The argument should be of the form -Wsan%s.\n", "PNBRQK");
			exit(1);
		} else if (letters[piece_index] != '\0') {
			fprintf(GlobalState.logfile, "Too many piece letters found with -Wsan%s.\n", letters);
			fprintf(GlobalState.logfile, "The argument should be of the form -Wsan%s.\n", "PNBRQK");
			exit(1);
		} else {
			/* Ok. */
...
Board *new_fen_board(const char *fen) {
	...
	static const Board initial_board = {{{OFF, OFF, OFF, OFF, OFF, OFF, OFF, OFF, OFF, OFF, OFF, OFF},
	...
	*new_board = initial_board;
	...
	col = FIRSTCOL;
	while (Ok && (*fen_char != ' ') && (*fen_char != '\0') && (rank >= FIRSTRANK)) {
		Piece piece;
		char ch = *fen_char;
		Colour colour;
		...
		if ((piece = is_FEN_piece(ch)) != EMPTY) {
			...
			if (col <= LASTCOL) {
				new_board->board[RankConvert(rank)][ColConvert(col)] = MAKE_COLOURED_PIECE(colour, piece);
				if (piece == KING) {
					if (colour == WHITE) {
	...
	if (*fen_char == '-') {
		...
		new_board->WKingCastle = new_board->WQueenCastle = new_board->BKingCastle = new_board->BQueenCastle = FALSE;
		fen_char++;
	} else {
	...
	if (*fen_char == '-') {
		...
		if (is_rank(*fen_char)) {
			 ...
			 */
			if (((new_board->to_move == WHITE) && (rank == '6')) || ((new_board->to_move == BLACK) && (rank == '3'))) {
				/* Consistent. */
 ...
 */
static Board *new_game_board(const char *fen) {
	Board *new_board;
	static const Board initial_board = {{{OFF, OFF, OFF, OFF, OFF, OFF, OFF, OFF, OFF, OFF, OFF, OFF},
	                                     {OFF, OFF, OFF, OFF, OFF, OFF, OFF, OFF, OFF, OFF, OFF, OFF},
...
static void print_square(Col col, Rank rank, const Board *board, FILE *outfp) {
	...
	switch ((int) coloured_piece) {
		...
		case B(ROOK):
		case B(QUEEN):
		case B(KING):putc(coloured_piece_to_SAN_letter(coloured_piece), outfp);
	        break;
		case EMPTY:putc('.', outfp);
	        break;
		case OFF:putc('?', outfp);
	        break;
		default: fprintf(GlobalState.logfile, "Attempt to print illegal square %c%c in print_square.\n", col, rank);
	        break;
	}
}
 ...
 */
static Boolean apply_move(Colour colour, Move *move_details, Board *board) {   /* Assume success. */
	Boolean Ok = TRUE;
	...
	if (determine_move_details(colour, move_details, board)) {
		...
		if (Ok) {
			move_details->check_status = king_is_in_check(board, OPPOSITE_COLOUR(colour));
...
* variation.
 */
static Boolean play_moves(Game *game_details,
                          Board *board,
                          Move *moves,
                          unsigned max_depth,
	 ...
	 * a match.
	 */
	while (game_ok && (next_move != NULL) && (game_matches || (board->move_number <= max_depth))) {
		if (*(next_move->move) != '\0') {
			/* See if there are any variations associated with this move. */
			if ((next_move->Variants != NULL) && GlobalState.keep_variations) {
				game_matches |= apply_variations(game_details, board, next_move->Variants, check_move_validity);
			}
			...
			if (check_move_validity) {
				if (apply_move(board->to_move, next_move, board)) {
					...
					if (GlobalState.add_ECO && !GlobalState.parsing_ECO_file) {
						...
						EcoLog *entry = eco_matches(board->hash_value, game_details->cumulative_hash_value, half_moves);
						if (entry != NULL) {
			...
			/* An empty move. */
			fprintf(GlobalState.logfile, "Internal error: Empty move in play_moves.\n");
			report_details(GlobalState.logfile);
			game_ok = FALSE;
			/* Work out where the error was. */
			error_ply = 2 * board->move_number - 1;
	...
	if (game_ok) {
		if (eco_match != NULL) {
			...
			if (eco_match->Variation_tag != NULL) {
				game_details->tags[VARIATION_TAG] = copy_string(eco_match->Variation_tag);
			}
			if (eco_match->Sub_Variation_tag != NULL) {
				game_details->tags[SUB_VARIATION_TAG] = copy_string(eco_match->Sub_Variation_tag);
			}
		}
	}
...
static void play_eco_moves(Game *game_details, Board *board, Move *moves) {
	...
	while (game_ok && (next_move != NULL)) {
		if (*(next_move->move) != '\0') {
			...
			/* An empty move. */
			fprintf(GlobalState.logfile, "Internal error: Empty move in play_eco_moves.\n");
			report_details(GlobalState.logfile);
			game_ok = FALSE;
			/* Work out where the error was. */
			error_ply = 2 * board->move_number - 1;
 ...
 * we are looking for.
 */
static Boolean apply_variations(const Game *game_details,
                                const Board *board,
                                Variation *variation,
                                Boolean check_move_validity) {   /* Force a match if we aren't looking for positional variations. */
	...
	while (variation != NULL) {
		...
		*copy_board = *board;
		 ...
		 */
		variation_matches |=
			play_moves(copy_game, copy_board, variation->moves, DEFAULT_POSITIONAL_DEPTH, check_move_validity);
...
static Boolean rewrite_SAN_string(Colour colour, Move *move_details, Board *board) {
	...
	if (move_details == NULL) {
		/* Shouldn't happen. */
		fprintf(GlobalState.logfile, "Internal error: NULL move details in rewrite_SAN_string.\n");
		Ok = FALSE;
		...
		switch (class) {
			case PAWN_MOVE:
			case ENPASSANT_PAWN_MOVE:
			case PAWN_MOVE_WITH_PROMOTION: move_list = find_pawn_moves(move_details->from_col,
			                                                           '0',
			                                                           to_col,
			                                                           to_rank,
			                                                           colour,
			...
			case PIECE_MOVE:
				switch (move_details->piece_to_move) {
					case KING:move_list = find_king_moves(to_col, to_rank, colour, board);
				        break;
					case QUEEN:move_list = find_queen_moves(to_col, to_rank, colour, board);
				        break;
					case ROOK:move_list = find_rook_moves(to_col, to_rank, colour, board);
				        break;
					case KNIGHT:move_list = find_knight_moves(to_col, to_rank, colour, board);
				        break;
					case BISHOP:move_list = find_bishop_moves(to_col, to_rank, colour, board);
				        break;
					default:fprintf(GlobalState.logfile, "Unknown piece move %s\n", move);
				        Ok = FALSE;
				        break;
				}
		        break;
			...
			case NULL_MOVE:
				...
				break;
			case UNKNOWN_MOVE:
			default: fprintf(GlobalState.logfile,
		...
		if (move_list != NULL) {
			move_list = exclude_checks(move_details->piece_to_move, colour, move_list, board);
		}
		if ((move_list == NULL) && (class != KINGSIDE_CASTLE) && (class != QUEENSIDE_CASTLE) && (class != NULL_MOVE)) {
			Ok = FALSE;
		}
			...
			switch (class) {
				...
				case PIECE_MOVE: {
					...
					if (move_list->next != NULL) {
						...
						for (possible = move_list; possible != NULL; possible = possible->next) {
			        ...
			        break;
				case KINGSIDE_CASTLE:strcpy((char *) new_move_str, "O-O");
			        break;
				case QUEENSIDE_CASTLE:strcpy((char *) new_move_str, "O-O-O");
			        break;
				case NULL_MOVE:strcpy((char *) new_move_str, (char *) NULL_MOVE_STRING);
			        break;
				case UNKNOWN_MOVE:
				default:Ok = FALSE;
			        break;
 ...
 * Return TRUE if the move is ok, FALSE otherwise.
 */
static Boolean rewrite_move(Colour colour,
                            Move *move_details,
                            Board *board,
                            Boolean null_move_found) {   /* Assume success. */
	Boolean Ok = TRUE;
...
static Boolean rewrite_moves(Board *board, Move *moves, Boolean null_move_found) {
	...
	while (game_ok && (move_details != NULL)) {
		if (*(move_details->move) != '\0') {
			...
			/* An empty move. */
			fprintf(GlobalState.logfile, "Internal error: Empty move in rewrite_moves.\n");
			report_details(GlobalState.logfile);
			game_ok = FALSE;
		}
	}
...
static Boolean position_matches(const Board *board) {
	...
	for (entry = codes_of_interest[ix]; !found && (entry != NULL); entry = entry->next) {
...
static void append_evaluation(Move *move_details, const Board *board) {
	...
	if (strlen(evaluation) > strlen(valueSpace)) {
		fprintf(GlobalState.logfile, "Overflow in evaluation space in append_evaluation()\n");
		exit(1);
	}
...
static double shannonEvaluation(const Board *board) {
	...
	shannonValue = (whitePieceCount - blackPieceCount) + (whiteMoveCount - blackMoveCount) * 0.1;
	return shannonValue;
}
