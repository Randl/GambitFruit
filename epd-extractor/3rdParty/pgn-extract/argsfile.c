...
static void usage_and_exit(void) {
	const char *help_data[] = {"-7 -- output only the seven tag roster for each game. Other tags (apart",
	                           ...
	                           "      refined division of games.", "      All files are opened in append mode.",
	                           ...
	                           "      Default is SAN.", "      -W means use the input format.",
	                           "      -Wcm is (a possibly obsolete) ChessMaster format.", "      -Wepd is EPD format.",
	                           "      -Wsan[PNBRQK] for language specific output.",
	                           "      -Whalg is hyphenated long algebraic.", "      -Wlalg is long algebraic.",
	                           ...
	                           "--addhashcode - output a HashCode tag", "--append - see -a", "--checkfile - see -c",
	                           "--checkmate - see -M", "--duplicates - see -d",
	                           "--evaluation - include a position evaluation after each move",
	                           "--fencomments - include a FEN string after each move",
	                           "--fuzzydepth plies - positional duplicates match", "--help - see -h",
	                           "--keepbroken - retain games with errors", "--linelength - see -w",
	                           "--markmatches - mark positional and material matches with a comment; see -t, -v, and -z",
	                           "--nochecks - don't output + and # after moves.", "--nocomments - see -C",
	                           "--noduplicates - see -D",
	                           "--nofauxep - don't output ep squares in FEN when the capture is not possible",
	                           "--nomovenumbers - don't output move numbers.", "--nonags - see -N",
	                           "--noresults - don't output results.", "--notags - don't output any tags.",
	                           "--nounique - see -U", "--novars - see -V", "--output - see -o",
	                           "--plylimit - limit the number of plies output.",
	                           "--selectonly N - only output the Nth matched game (N > 0)", "--seven - see -7",
	...
	fprintf(GlobalState.logfile,
	        ...
	        CURRENT_VERSION,
	        __DATE__);
	fprintf(GlobalState.logfile, "Copyright (C) 1994-2015 David J. Barnes (d.j.barnes@kent.ac.uk)\n");
		...
		while ((line = read_line(fp)) != NULL) {
			...
			if (nexttype == NO_ARGUMENT_MATCH) {
				if (*line == argument_prefix[0]) {
					...
					/* Handle the line. */
					switch (linetype) {
						case MOVES_ARGUMENT:add_textual_variation_from_line(line);
					        break;
						case POSITIONS_ARGUMENT:add_positional_variation_from_line(line);
					        break;
						case TAGS_ARGUMENT:process_tag_line(infile, line);
					        break;
						case TAG_ROSTER_ARGUMENT:process_roster_line(line);
					        break;
						case ENDINGS_ARGUMENT:process_ending_line(line);
					        (void) free(line);
					        break;
						default:
							fprintf(GlobalState.logfile,
					 ...
					 * current linetype.
					 */
					fprintf(GlobalState.logfile, "Missing argument type for line %s in the argument file.\n", line);
					exit(1);
				}
			} else {
				switch (nexttype) {
					...
					case OUTPUT_FORMAT_ARGUMENT: process_argument(line[argument_prefix_len],
					...
					case USE_VIRTUAL_HASH_TABLE_ARGUMENT:process_argument(line[argument_prefix_len], "");
				        linetype = NO_ARGUMENT_MATCH;
				        break;
					...
					case TAGS_ARGUMENT:
					case TAG_ROSTER_ARGUMENT: process_argument(line[argument_prefix_len],
					                                           &line[argument_prefix_len + 1]);
				        /* Apply this type to subsequent lines. */
				        linetype = nexttype;
				        break;
					default:linetype = nexttype;
				        break;
				}
				(void) free(line);
			}
...
static ArgType classify_arg(const char *line) {
	 ...
	 *            :-argument
	 */
	if ((strncmp(line, argument_prefix, argument_prefix_len) == 0) && (line_length >= min_argument_length)) {
		...
		switch (argument_letter) {
			...
			case LONG_FORM_ARGUMENT:return (ArgType) argument_letter;
			default: fprintf(GlobalState.logfile, "Unrecognized argument: %s in the argument file.\n", line);
		        exit(1);
 ...
 * it must be copied.
 */
void process_argument(char arg_letter, const char *associated_value) {   /* Provide an alias for associated_value because it will
	...
	switch (arg_letter) {
		...
		case DUPLICATES_FILE_ARGUMENT:
	        ...
	        break;
		case USE_ECO_FILE_ARGUMENT:GlobalState.add_ECO = TRUE;
	        if (*filename != '\0') {
		...
		case ECO_OUTPUT_LEVEL_ARGUMENT: {
			...
			if (GlobalState.output_filename != NULL) {
				...
				fprintf(GlobalState.logfile, "-%c requires a number attached, e.g., -%c1.\n", arg_letter, arg_letter);
				exit(1);
		...
		case CHECK_FILE_ARGUMENT:
			if (*filename != '\0') {
				...
				if ((len > strlen(suffix)) && (stringcompare(&filename[len - strlen(suffix)], suffix) == 0)) {
		...
		case BOUNDS_ARGUMENT: {
			...
			if (Ok && (sscanf(bound, "%u", &value) == 1)) {
				...
				switch (which) {
					case 'e':GlobalState.lower_move_bound = value;
				...
				fprintf(GlobalState.logfile, "-%c should be in the form -%c[elu]number.\n", arg_letter, arg_letter);
				Ok = FALSE;
			}
			if (!Ok) {
				exit(1);
		...
		case GAMES_PER_FILE_ARGUMENT:
			if (GlobalState.ECO_level > 0) {
				...
				fprintf(GlobalState.logfile,
				        ...
				        GlobalState.output_filename);
				exit(1);
			} else if (sscanf(associated_value, "%u", &GlobalState.games_per_file) != 1) {
				fprintf(GlobalState.logfile, "-%c should be followed by an unsigned integer.\n", arg_letter);
				exit(1);
			} else {
				/* Value set. */
			}
		...
		case LINE_WIDTH_ARGUMENT: { /* Specify an output line width. */
	        ...
	        break;
		case HELP_ARGUMENT:usage_and_exit();
	        break;
		case OUTPUT_FORMAT_ARGUMENT:
			 ...
			 */
		{
	        ...
	        break;
		case SEVEN_TAG_ROSTER_ARGUMENT:
			if (GlobalState.tag_output_format == ALL_TAGS || GlobalState.tag_output_format == SEVEN_TAG_ROSTER) {
				GlobalState.tag_output_format = SEVEN_TAG_ROSTER;
			} else {
				fprintf(GlobalState.logfile, "-%c clashes with another argument.\n", SEVEN_TAG_ROSTER_ARGUMENT);
				exit(1);
			}
	        break;
		case DONT_KEEP_COMMENTS_ARGUMENT:GlobalState.keep_comments = FALSE;
	        break;
		case DONT_KEEP_DUPLICATES_ARGUMENT:
	        ...
	        break;
		case DONT_MATCH_PERMUTATIONS_ARGUMENT:GlobalState.match_permutations = FALSE;
	        break;
		case DONT_KEEP_NAGS_ARGUMENT:GlobalState.keep_NAGs = FALSE;
	        break;
		case OUTPUT_FEN_STRING_ARGUMENT:
		...
		case MATCH_CHECKMATE_ARGUMENT:
	        ...
	        break;
		case SUPPRESS_ORIGINALS_ARGUMENT:GlobalState.suppress_originals = TRUE;
	        break;
		case DONT_KEEP_VARIATIONS_ARGUMENT:GlobalState.keep_variations = FALSE;
	        break;
		case USE_VIRTUAL_HASH_TABLE_ARGUMENT:GlobalState.use_virtual_hash_table = TRUE;
	        break;
		...
		case ENDINGS_ARGUMENT:
	        ...
	        break;
		default: fprintf(GlobalState.logfile, "Unrecognized argument -%c\n", arg_letter);
	}
}
	...
	if (stringcompare(argument, "addhashcode") == 0) {
		...
		if (sscanf(associated_value, "%d", &depth) == 1) {
			if (depth >= 0) {
				...
				fprintf(GlobalState.logfile, "--%s requires a number greater than or equal to zero.\n", argument);
				exit(1);
			}
		} else {
		...
		if (*associated_value != '\0') {
			...
			fprintf(GlobalState.logfile, "--markmatches requires a comment string following it.\n");
			exit(1);
		}
		return 2;
		...
		if (GlobalState.tag_output_format == ALL_TAGS || GlobalState.tag_output_format == NO_TAGS) {
			GlobalState.tag_output_format = NO_TAGS;
		} else {
			fprintf(GlobalState.logfile, "--notags clashes with another argument.\n");
			exit(1);
		}
		return 1;
		...
		if (sscanf(associated_value, "%lu", &selection) == 1) {
			if (selection > 0) {
				...
				fprintf(GlobalState.logfile, "--%s requires a number greater than zero.\n", argument);
				exit(1);
			}
		} else {
			fprintf(GlobalState.logfile, "--%s requires a number greater than zero following it.\n", argument);
			exit(1);
		}
		return 2;
