...
Piece is_piece(const unsigned char *move) {
	...
	switch (*move) {
		...
		case 'q':
		case 'D': /* Dutch/German. */
		case RUSSIAN_QUEEN:piece = QUEEN;
	        break;
		case 'R':
		case 'r':
		case 'T': /* Dutch/German. */
		case RUSSIAN_ROOK:piece = ROOK;
	        break;
		case 'N':
		case 'n':
		case 'P': /* Dutch. */
 ...
 * of lexical analysis.
 */
Move *decode_move(const unsigned char *move_string) {   /* The four components of the co-ordinates when known. */
	...
	if (is_col(*move)) {
		...
		if (is_rank(*move)) {
			...
			if (is_col(*move)) {
				...
				if (is_rank(*move)) {
					...
					if ((from_col != 'b') && (from_col != (to_col + 1)) && (from_col != (to_col - 1))) {
						Ok = FALSE;
					}
				} else {
					/* Check the sanity of this. */
		...
		if ((RUSSIAN_PIECE_CHECK(*move) == RUSSIAN_KNIGHT_OR_KING) && (piece_to_move == KING)) {
			move++;
		}
		move++;
		if (is_rank(*move)) {
	...
	if (Ok && class != NULL_MOVE) {
		...
		} else if (((strcmp((const char *) move, "ep") == 0) || (strcmp((const char *) move, "e.p.") == 0))
			&& (class == PAWN_MOVE)) {
			/* These are ok. */
...
Boolean move_seems_valid(const unsigned char *move_string) {
	...
	if (is_col(*move)) {
		...
		class = PIECE_MOVE;
		/* Check for a two-character piece. */
		if ((RUSSIAN_PIECE_CHECK(*move) == RUSSIAN_KNIGHT_OR_KING) && (is_piece(move) == KING)) {
			move++;
		}
		move++;
		if (is_rank(*move)) {
