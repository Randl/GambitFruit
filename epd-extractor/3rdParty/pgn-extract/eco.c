 ...
 */
void save_eco_details(Game game_details, unsigned number_of_half_moves) {
	...
	for (entry = EcoTable[ix]; (entry != NULL) && can_save; entry = entry->next) {
		if ((entry->required_hash_value == game_details.final_hash_value) && (entry->half_moves == number_of_half_moves)
			&& (entry->cumulative_hash_value == game_details.cumulative_hash_value)) {
			const char *tag = entry->ECO_tag, *opening = entry->Opening_tag, *variation = entry->Variation_tag;
			if (tag == NULL) {
				tag = "";
			}
			if (opening == NULL) {
	...
	if (can_save) {
		...
		if (game_details.tags[OPENING_TAG] != NULL) {
			if ((last_entry != NULL) && (last_entry->Opening_tag != NULL)
				&& (strcmp(last_entry->Opening_tag, game_details.tags[OPENING_TAG]) == 0)) {
				/* Share the last entry's tag. */
		...
		if (game_details.tags[SUB_VARIATION_TAG] != NULL) {
			entry->Sub_Variation_tag = copy_string(game_details.tags[SUB_VARIATION_TAG]);
		} else {
 ...
 */
EcoLog *eco_matches(HashCode current_hash_value, HashCode cumulative_hash_value, unsigned half_moves_played) {
	...
	if (half_moves_played <= maximum_half_moves) {
		...
		for (entry = EcoTable[ix]; entry != NULL; entry = entry->next) {
			if (entry->required_hash_value == current_hash_value) {
				...
				if (half_moves_played == entry->half_moves && entry->cumulative_hash_value == cumulative_hash_value) {
					return entry;
				} else if (abs(half_moves_played - entry->half_moves) <= ECO_HALF_MOVE_LIMIT) {
 ...
 */
FILE *open_eco_output_file(EcoDivision ECO_level, const char *eco) {   /* Allow space for the maximum number of
	...
	if ((eco == NULL) || !isalpha((int) *eco)) {
		...
		fprintf(GlobalState.logfile, "Internal error: ECO division in open_eco_output_file\n");
