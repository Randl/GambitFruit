...
typedef enum {
    EXACTLY, NUM_OR_MORE, NUM_OR_LESS, SAME_AS_OPPONENT, NOT_SAME_AS_OPPONENT, LESS_THAN_OPPONENT, MORE_THAN_OPPONENT,
...
static const char *extract_combination(const char *p, Occurs *p_occurs, int *p_number, const char *line) {
	Boolean Ok = TRUE;
	...
	if (Ok) {
		...
		switch (*p) {
			case '*':number = 0;
		        ...
		        p++;
		        break;
			case '+':occurs = NUM_OR_MORE;
		        p++;
		        break;
			case '-':occurs = NUM_OR_LESS;
		        p++;
		        break;
			case '?':number = 1;
		        occurs = NUM_OR_LESS;
 ...
 */
static const char *extract_piece_information(const char *line, Ending_details *details, Colour colour) {
	const char *p = line;
	Boolean Ok = TRUE;
	...
	if (Ok) {
		...
		if ((details->num_minor_pieces[colour] > 0) || (details->minor_occurs[colour] != EXACTLY)) {
			/* Warn about use of BISHOP and KNIGHT letters. */
			if ((details->num_pieces[colour][BISHOP] > 0) || (details->occurs[colour][BISHOP] != EXACTLY)
				|| (details->num_pieces[colour][KNIGHT] > 0) || (details->occurs[colour][KNIGHT] != EXACTLY)) {
				fprintf(GlobalState.logfile,
				        "Warning: the mixture of minor pieces in %s is not guaranteed to work.\n",
				        line);
				fprintf(GlobalState.logfile, "In a single set it is advisable to stick to either L or B and/or N.\n");
			}
		}
		return p;
	} else {
...
static Boolean decompose_line(const char *line, Ending_details *details) {
	const char *p = line;
	Boolean Ok = TRUE;
...
static Boolean piece_match(int num_available, int num_to_find, int num_opponents, Occurs occurs) {
	Boolean match = FALSE;

	switch (occurs) {
		case EXACTLY:match = num_available == num_to_find;
	        break;
		case NUM_OR_MORE:match = num_available >= num_to_find;
	        break;
		case NUM_OR_LESS:match = num_available <= num_to_find;
	        break;
		case SAME_AS_OPPONENT:match = num_available == num_opponents;
	        break;
		case NOT_SAME_AS_OPPONENT:match = num_available != num_opponents;
	        break;
		case LESS_THAN_OPPONENT:match = (num_available + num_to_find) <= num_opponents;
	        break;
		case MORE_THAN_OPPONENT:match = (num_available - num_to_find) >= num_opponents;
	        break;
		...
		case MORE_EQ_THAN_OPPONENT:
			...
			match = (num_available - num_to_find) == num_opponents;
	        break;
		default: fprintf(GlobalState.logfile, "Inconsistent state %d in piece_match.\n", occurs);
	        match = FALSE;
	}
	return match;
}
 ...
 */
static Boolean piece_set_match(Ending_details *details_to_find,
                               int num_pieces[2][NUM_PIECE_VALUES],
                               Colour game_colour,
	...
	if (match) {
		...
		if ((num_to_find > 0) || (occurs != EXACTLY)) {
			int num_available = num_pieces[game_colour][BISHOP] + num_pieces[game_colour][KNIGHT];
			int num_opponents =
				num_pieces[OPPOSITE_COLOUR(game_colour)][BISHOP] + num_pieces[OPPOSITE_COLOUR(game_colour)][KNIGHT];
 ...
 */
static Boolean ending_match(Ending_details *details_to_find, int num_pieces[2][NUM_PIECE_VALUES], Colour game_colour) {
	Boolean match = TRUE;
...
static Boolean look_for_ending(Move *moves, Ending_details *details_to_find) {
	...
	int num_pieces[2][NUM_PIECE_VALUES] = {
		...
		/*   P N B R Q K */
		{0, 0, 8, 2, 2, 2, 1, 1}, {0, 0, 8, 2, 2, 2, 1, 1}};
	...
	while (game_ok && (next_move != NULL) && !game_matches) {
		 ...
		 */
		game_matches =
			ending_match(details_to_find, num_pieces, WHITE) | ending_match(details_to_find, num_pieces, BLACK);
		if (!game_matches) {
			if (*(next_move->move) != '\0') {
				...
				/* An empty move. */
				fprintf(GlobalState.logfile, "Internal error: Empty move in look_for_ending.\n");
				game_ok = FALSE;
			}
		} else {
			/* Match.
...
Boolean check_for_ending(Move *moves) {   /* Match if there are no endings to match. */
	Boolean matches = (endings_to_match == NULL) ? TRUE : FALSE;
	Ending_details *details;

	for (details = endings_to_match; !matches && (details != NULL); details = details->next) {
