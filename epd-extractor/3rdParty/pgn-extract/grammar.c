...
void init_game_header(void) {
	...
	GameHeader.Tags = (char **) MallocOrDie(GameHeader.header_tags_length * sizeof(*GameHeader.Tags));
...
void increase_game_header_tags_length(unsigned new_length) {
	...
	if (new_length <= GameHeader.header_tags_length) {
		fprintf(GlobalState.logfile, "Internal error: inappropriate length %d ", new_length);
		fprintf(GlobalState.logfile, " passed to increase_game_header_tags().\n");
		exit(1);
	}
	GameHeader.Tags = (char **) ReallocOrDie((void *) GameHeader.Tags, new_length * sizeof(*GameHeader.Tags));
...
static void check_result(char **Tags, const char *terminating_result) {
	...
	if (terminating_result != NULL) {
		if ((result_tag == NULL) || (*result_tag == '\0') || (strcmp(result_tag, "?") == 0)) {
			/* Use a copy of terminating result. */
			result_tag = copy_string(terminating_result);
			Tags[RESULT_TAG] = result_tag;
		} else if ((result_tag != NULL) && (strcmp(terminating_result, "*") != 0)
			&& (strcmp(result_tag, terminating_result) != 0)) {
			print_error_context(GlobalState.logfile);
			fprintf(GlobalState.logfile, "Inconsistent result strings in the following game.\n");
...
static FILE *select_output_file(StateInfo *GameState, const char *eco) {
	if (GameState->games_per_file > 0) {
		if (GameState->games_per_file == 1 || (GameState->num_games_matched % GameState->games_per_file) == 1) {
			...
			sprintf(filename, "%u%s", GameState->next_file_number, output_file_suffix(GameState->output_format));
 ...
 */
static Boolean finished_processing(void) {
	return (GlobalState.matching_game_number > 0 && GlobalState.num_games_matched == GlobalState.matching_game_number);
}
 ...
 */
static Boolean ParseGame(Move **returned_move_list) {   /* Boolean something_found = FALSE; */
	CommentList *prefix_comment;
	Move *move_list = NULL;
	char *result;
...
Boolean ParseOptTagList(void) {
	...
	if (something_found) {
		...
		if ((GameHeader.Tags[SETUP_TAG] != NULL) && (strcmp(GameHeader.Tags[SETUP_TAG], "1") == 0)) {
	...
	if (current_symbol == TAG) {
		...
		if (current_symbol == STRING) {
			...
			if (tag_index < GameHeader.header_tags_length) {
				...
				print_error_context(GlobalState.logfile);
				fprintf(GlobalState.logfile, "Internal error: Illegal tag index %d for %s\n", tag_index, tag_string);
				exit(1);
			}
			current_symbol = next_token();
		} else {
...
static void DealWithGame(Move *move_list) {
	 ...
	 * been checked.
	 */
	if (CheckTagDetailsNotECO(current_game.tags, current_game.tags_length) && apply_move_list(&current_game, &plycount)
		&& check_move_bounds(plycount) && check_textual_variations(current_game) && check_for_ending(current_game.moves)
		&& check_for_only_checkmate(current_game.moves) && CheckECOTag(current_game.tags)) {
		...
		if ((original_filename == NULL) && GlobalState.suppress_originals) {
			...
			if (GlobalState.check_only) {
				...
				FILE *outputfile = select_output_file(&GlobalState, current_game.tags[ECO_TAG]);

				/* See if we wish to separate out duplicates. */
				if ((original_filename != NULL) && (GlobalState.duplicate_file != NULL)) {
	...
	if (!game_output && (GlobalState.non_matching_file != NULL) && GlobalState.current_file_type != CHECKFILE) {
