...
void init_duplicate_hash_table(void) {
	...
	if (GlobalState.use_virtual_hash_table) {
		VirtualLogTable = (LogHeaderEntry *) MallocOrDie(LOG_TABLE_SIZE * sizeof(*VirtualLogTable));
...
static int retrieve_virtual_entry(long ix, VirtualHashLog *entry) {
	if (hash_file == NULL) {
		...
		fprintf(GlobalState.logfile, "Fseek error to %ld in retrieve_virtual_entry\n", ix);
		return 0;
	} else if (fread((void *) entry, sizeof(*entry), 1, hash_file) != 1) {
		fprintf(GlobalState.logfile, "Fread error from %ld in retrieve_virtual_entry\n", ix);
		return 0;
	} else {
		return 1;
	}
...
static int write_virtual_entry(long where, const VirtualHashLog *entry) {
	if (fseek(hash_file, where, SEEK_SET) != 0) {
		...
		fprintf(GlobalState.logfile, "Fwrite error from %ld in write_virtual_entry\n", where);
		return 0;
	} else {
		/* Written ok. */
		return 1;
...
static const char *previous_virtual_occurance(Game game_details) {
	...
	if (GlobalState.suppress_duplicates || GlobalState.suppress_originals || GlobalState.duplicate_file != NULL) {
...
const char *previous_occurance(Game game_details, int plycount) {
	...
	if (GlobalState.use_virtual_hash_table) {
		...
		/* Are we keeping this information? */
		if (GlobalState.suppress_duplicates || GlobalState.suppress_originals || GlobalState.fuzzy_match_duplicates
			|| GlobalState.duplicate_file != NULL) {
			for (entry = LogTable[ix]; !duplicate && (entry != NULL); entry = entry->next) {
				if (entry->final_hash_value == game_details.final_hash_value
					&& entry->cumulative_hash_value == game_details.cumulative_hash_value) {
					/* An exact match. */
					duplicate = TRUE;
			...
			if (!duplicate) {
				...
				if (!GlobalState.fuzzy_match_duplicates) {
					...
					entry->final_hash_value = game_details.final_hash_value;
					entry->cumulative_hash_value = game_details.cumulative_hash_value;
				} else if (GlobalState.fuzzy_match_depth > 0 && plycount >= GlobalState.fuzzy_match_depth) {
