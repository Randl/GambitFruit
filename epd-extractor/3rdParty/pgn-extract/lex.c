...
static Boolean extract_yytext(const unsigned char *symbol_start, const unsigned char *linep);
...
static TagName make_new_tag(const char *tag) {
	...
	tag_list_length++;
	TagList = (const char **) ReallocOrDie((void *) TagList, tag_list_length * sizeof(*TagList));
 ...
 */
static LinePair gather_possible_numeric(char *line, unsigned char *linep, char initial_digit) {
	LinePair resulting_line;
	...
	if (line != NULL) {
		...
		if (len > 0) {
			...
			if (tag_item >= 0 && ((unsigned) tag_item) < tag_list_length) {
				...
				fprintf(GlobalState.logfile, "Internal error: invalid tag index %d in gather_tag.\n", tag_item);
				exit(1);
			}
		} else {
...
static Boolean extract_yytext(const unsigned char *symbol_start,
                              const unsigned char *linep) {   /* Whether the string fitted. */
	Boolean Ok = TRUE;
		...
		if (line == NULL) {
			...
			switch (token) {
				case WHITESPACE:
			        ...
			        token = NO_TOKEN;
			        break;
				case TAG_START:resulting_line = gather_tag(line, linep);
			        /* Pick up where we are now. */
			        line = resulting_line.line;
			        linep = resulting_line.linep;
			        token = resulting_line.token;
			        break;
				case TAG_END:token = NO_TOKEN;
			        break;
				case DOUBLE_QUOTE:resulting_line = gather_string(line, linep);
			        /* Pick up where we are now. */
			        line = resulting_line.line;
			        linep = resulting_line.linep;
			        token = resulting_line.token;
			        break;
				case COMMENT_START:resulting_line = gather_comment(line, linep);
			        /* Pick up where we are now. */
			        line = resulting_line.line;
			        linep = resulting_line.linep;
			        token = resulting_line.token;
				...
				case ERROR_TOKEN:
			        ...
			        while (ChTab[(unsigned) *linep] == ERROR_TOKEN)
				        linep++;
			        break;
				case OPERATOR:print_error_context(GlobalState.logfile);
 ...
 */
static FILE_LIST list_of_files = {(const char **) NULL, (SourceFileType *) NULL, 0, 0};
...
void add_filename_list_from_file(FILE *fp, SourceFileType file_type) {
	if ((list_of_files.files == NULL) || (list_of_files.max_files == 0)) {
		 ...
		 */
		list_of_files.files = (const char **) MallocOrDie((INIT_LIST_SPACE + 1) * sizeof(const char *));
		list_of_files.file_type = (SourceFileType *) MallocOrDie((INIT_LIST_SPACE + 1) * sizeof(SourceFileType));
...
void add_filename_to_source_list(const char *filename, SourceFileType file_type) {    /* Where to put it. */
		...
		if ((list_of_files.files == NULL) || (list_of_files.max_files == 0)) {
			...
			list_of_files.files = (const char **) realloc((void *) list_of_files.files,
			                                              (list_of_files.max_files + MORE_LIST_SPACE + 1)
				                                              * sizeof(const char *));
			list_of_files.file_type = (SourceFileType *) realloc((void *) list_of_files.file_type,
			                                                     (list_of_files.max_files + MORE_LIST_SPACE + 1)
	...
	if (file_type == CHECKFILE) {
		...
		for (location = 0; (location < list_of_files.num_files) && (list_of_files.file_type[location] == CHECKFILE);
		     location++) {
			/* Do nothing. */
		}
...
char *next_input_line(FILE *fp) {   /* Retain each line in turn, so as to be able to free it. */
	static char *line = NULL;
