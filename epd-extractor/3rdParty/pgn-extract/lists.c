...
static void extend_tag_list_length(int new_length) {
	if (new_length < tag_list_length) {
		fprintf(GlobalState.logfile, "Internal error: inappropriate call to extend_tag_list_length().\n");
		...
		int i;
		TagLists = (StringArray *) ReallocOrDie((void *) TagLists, new_length * sizeof(*TagLists));
...
static int add_to_taglist(const char *str, StringArray *list) {
	...
	if (list->num_allocated_elements == list->num_used_elements) {
		...
		if (list->num_allocated_elements == 0) {
			...
			list->tag_strings = (TagSelection *) MallocOrDie((INIT_LIST_SPACE + 1) * sizeof(TagSelection));
...
static Boolean soundex_tag(int tag) {
	...
	switch (tag) {
		...
		case EVENT_TAG:
		case SITE_TAG:
		case ANNOTATOR_TAG:use_soundex = TRUE;
	        break;
	}
	return use_soundex;
}
...
void extract_tag_argument(const char *argstr) {
	...
	switch (*argstr) {
		case 'a':add_tag_to_list(ANNOTATOR_TAG, arg, NONE);
	        break;
		case 'b':add_tag_to_list(BLACK_TAG, arg, NONE);
	        break;
		case 'd':add_tag_to_list(DATE_TAG, arg, NONE);
	        break;
		case 'e':add_tag_to_list(ECO_TAG, arg, NONE);
	        break;
		case 'h':add_tag_to_list(HASHCODE_TAG, arg, NONE);
	        break;
		case 'p':add_tag_to_list(PSEUDO_PLAYER_TAG, arg, NONE);
	        break;
		case 'r':add_tag_to_list(RESULT_TAG, arg, NONE);
	        break;
		case 'w':add_tag_to_list(WHITE_TAG, arg, NONE);
	        break;
		default: fprintf(GlobalState.logfile, "Unknown type of tag extraction argument: %s\n", argstr);
	        exit(1);
	        break;
	}
}
...
static Boolean CheckDate(const char *date_string, StringArray *list) {
	unsigned list_index;
	...
	for (list_index = 0; list_index < list->num_used_elements; list_index++) {
		...
		if (operator != NONE) {
			...
			if ((sscanf(list_string, "%u", &list_date) == 1) && (sscanf(date_string, "%u", &game_date) == 1)
				&& (game_date > MINDATE) && (game_date < MAXDATE)) {
				Boolean matches;
				switch (operator) {
					case LESS_THAN:matches = game_date < list_date;
				        break;
					case LESS_THAN_OR_EQUAL_TO:matches = game_date <= list_date;
				        break;
					case GREATER_THAN:matches = game_date > list_date;
				        break;
					case GREATER_THAN_OR_EQUAL_TO:matches = game_date >= list_date;
				        break;
					case EQUAL_TO:matches = game_date == list_date;
				        break;
					case NOT_EQUAL_TO:matches = game_date != list_date;
				        break;
					case NONE:
					default:
...
static Boolean CheckElo(const char *elo_string, StringArray *list) {
	unsigned list_index;
	Boolean wanted = TRUE;
	...
	for (list_index = 0; (list_index < list->num_used_elements) && wanted; list_index++) {
		...
		if (operator != NONE) {
			...
			if ((sscanf(list_string, "%u", &list_elo) == 1) && (sscanf(elo_string, "%u", &game_elo) == 1)) {
				switch (operator) {
					case LESS_THAN:wanted = game_elo < list_elo;
				        break;
					case LESS_THAN_OR_EQUAL_TO:wanted = game_elo <= list_elo;
				        break;
					case GREATER_THAN:wanted = game_elo > list_elo;
				        break;
					case GREATER_THAN_OR_EQUAL_TO:wanted = game_elo >= list_elo;
				        break;
					case EQUAL_TO:wanted = game_elo == list_elo;
				        break;
					case NOT_EQUAL_TO:wanted = game_elo != list_elo;
				        break;
					case NONE:
...
Boolean CheckTagDetailsNotECO(char *Details[], int num_details) {
	...
	if (GlobalState.check_tags) {
		...
		if (num_details < tag_list_length) {
			fprintf(GlobalState.logfile, "Internal error: mismatch in tag set lengths in ");
		...
		if (TagLists[PSEUDO_PLAYER_TAG].num_used_elements != 0) {
			...
			if (Details[WHITE_TAG] != NULL) {
				wanted = CheckList(WHITE_TAG, Details[WHITE_TAG], &TagLists[PSEUDO_PLAYER_TAG]);
				/* If we didn't find a player there, try for the opponent. */
				if (!wanted && (Details[BLACK_TAG] != NULL)) {
					wanted = CheckList(BLACK_TAG, Details[BLACK_TAG], &TagLists[PSEUDO_PLAYER_TAG]);
				}
			...
			if (Details[WHITE_ELO_TAG] != NULL) {
				wanted = CheckElo(Details[WHITE_ELO_TAG], &TagLists[PSEUDO_ELO_TAG]);
				/* If we didn't find a player there, try for the opponent. */
				if (!wanted && (Details[BLACK_ELO_TAG] != NULL)) {
					wanted = CheckElo(Details[BLACK_ELO_TAG], &TagLists[PSEUDO_ELO_TAG]);
				}
