...
static Boolean textual_variation_match(const char *variation_move, const unsigned char *actual_move);
...
static variation_list *compose_variation(char *line) {
	...
	move_list = (variant_move *) MallocOrDie(INIT_MOVE_NUMBER * sizeof(*move_list));
	...
	while (move != NULL) {
		if ((move = strip_move_number(move)) == NULL) {
			...
			if (num_moves == max_moves) {
				move_list =
					(variant_move *) realloc((void *) move_list, (max_moves + MOVE_INCREMENT) * sizeof(*move_list));
				if (move_list == NULL) {
...
static Boolean straight_match(Move *current_game_head, variation_list variation) {
	...
	while (matches && (next_move != NULL) && (move_index < variation.length)) {
		...
		if (*(moves_of_the_variation[move_index].move) == ANY_MOVE) {
			...
			this_move_matches = textual_variation_match(moves_of_the_variation[move_index].move, next_move->move);
...
static Boolean permutation_match(Move *current_game_head, variation_list variation) {
	 ...
	 * the current game.
	 */
	if ((variation.num_white_disallowed_moves > 0) || (variation.num_black_disallowed_moves > 0)) {
		 ...
		 * the game.
		 */
		while ((!disallowed_move_found) && (tested_moves < variation.length) && (next_move != NULL)) {
			...
			while ((!disallowed_move_found) && (variant_index < variation.length)) {
				if ((*moves_of_the_variation[variant_index].move == DISALLOWED_MOVE)
					&& (textual_variation_match(moves_of_the_variation[variant_index].move, next_move->move))) {
					/* Found one. */
...
Boolean check_textual_variations(Game game_details) {
	...
	if (games_to_keep != NULL) {
		for (variation = games_to_keep; (variation != NULL) && !wanted; variation = variation->next) {
...
Boolean check_move_bounds(unsigned plycount) {
	...
	if (GlobalState.check_move_bounds) {
		in_bounds =
			(GlobalState.lower_move_bound <= number_of_moves) && (number_of_moves <= GlobalState.upper_move_bound);
	}
	return in_bounds;
}
