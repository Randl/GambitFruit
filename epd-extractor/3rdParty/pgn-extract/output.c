...
static Boolean print_move(FILE *outputfile,
                          unsigned move_number,
                          unsigned print_move_number,
...
static void print_algebraic_game
	(Game current_game, FILE *outputfile, unsigned move_number, Boolean white_to_move, Board *final_board);
static void print_epd_game
	(Game current_game, FILE *outputfile, unsigned move_number, Boolean white_to_move, Board *final_board);
static void print_epd_move_list
	(Game current_game, FILE *outputfile, unsigned move_number, Boolean white_to_move, Board *final_board);
 ...
 */
static int DefaultTagOrder[] = {EVENT_TAG, SITE_TAG, DATE_TAG, ROUND_TAG, WHITE_TAG, BLACK_TAG, RESULT_TAG,
#if 1
	/* @@@ Consider omitting some of these from the default ordering,
	 * and allow the output order to be determined from the
	 * input order.
	 */
	                            WHITE_TITLE_TAG, BLACK_TITLE_TAG, WHITE_ELO_TAG, BLACK_ELO_TAG, WHITE_USCF_TAG,
	                            BLACK_USCF_TAG, WHITE_TYPE_TAG, BLACK_TYPE_TAG, WHITE_NA_TAG, BLACK_NA_TAG, ECO_TAG,
	                            NIC_TAG, OPENING_TAG, VARIATION_TAG, SUB_VARIATION_TAG, LONG_ECO_TAG, TIME_CONTROL_TAG,
	                            ANNOTATOR_TAG, EVENT_DATE_TAG, EVENT_SPONSOR_TAG, SECTION_TAG, STAGE_TAG, BOARD_TAG,
	                            TIME_TAG, UTC_DATE_TAG, UTC_TIME_TAG, SETUP_TAG, FEN_TAG, TERMINATION_TAG, MODE_TAG,
	                            PLY_COUNT_TAG,
#endif
...
OutputFormat which_output_format(const char *arg) {
	...
	struct {
	    const char *arg;
	    OutputFormat format;
	} formats[] = {{"san", SAN}, {"SAN", SAN}, {"epd", EPD}, {"EPD", EPD}, {"lalg", LALG}, {"halg", HALG}, {"CM", CM},
	               {"LALG", LALG}, {"HALG", HALG}, {"ELALG", ELALG}, {"elalg", ELALG}, {"XLALG", XLALG},
	               {"xlalg", XLALG}, {"uci", UCI}, {"cm", CM}, {"", SOURCE},
	...
	for (i = 0; formats[i].arg != NULL; i++) {
		...
		if (strncmp(arg, format_prefix, format_prefix_len) == 0) {
			 ...
			 */
			if ((format == SAN || format == ELALG || format == XLALG) && (strlen(arg) > format_prefix_len)) {
...
const char *output_file_suffix(OutputFormat format) {
	...
	switch (format) {
		...
		case HALG:
		case ELALG:
		case XLALG:return PGN_suffix;
		case EPD:return EPD_suffix;
		case CM:return CM_suffix;
		default:return PGN_suffix;
	}
}
...
static void show_tags(FILE *outfp, char **Tags, int tags_length) {
	 ...
	 */
	char **copy_of_tags = (char **) MallocOrDie(tags_length * sizeof(*copy_of_tags));
	int i;
...
static void print_comment_list(FILE *fp, CommentList *comment_list) {
	...
	for (next_comment = comment_list; next_comment != NULL; next_comment = next_comment->next) {
...
static void print_move_list(FILE *outputfile,
                            unsigned move_number,
                            Boolean white_to_move,
                            const Move *move_details,
	...
	if (GlobalState.output_ply_limit >= 0 && plies > GlobalState.output_ply_limit) {
		keepPrinting = FALSE;
	} else {
		keepPrinting = TRUE;
	}
	...
	while (move != NULL && keepPrinting) {
		...
		print_move_number = print_move(outputfile, move_number, print_move_number, white_to_move, move);

		/* See if there is a result attached.  This may be attached either
		 * to a move or a comment.
		 */
		if (!GlobalState.check_only && (move != NULL) && (move->terminating_result != NULL)) {
...
static Boolean print_move(FILE *outputfile,
	...
	if (move_details == NULL) {
		...
		if (GlobalState.check_only) {
			...
			if (*move_text != '\0') {
				if (GlobalState.keep_move_numbers && (white_to_move || print_move_number)) {
				...
				switch (output_format) {
						...
						*algebraic = '\0';
						switch (move_details->class) {
							...
							case PAWN_MOVE_WITH_PROMOTION:
						        ...
						        break;
							case NULL_MOVE: strcpy(algebraic, NULL_MOVE_STRING);
						        break;
							case UNKNOWN_MOVE:strcpy(algebraic, "???");
						        break;
						}
						if (GlobalState.keep_checks) {
							switch (move_details->check_status) {
								case NOCHECK:break;
								case CHECK:strcat(algebraic, "+");
							        break;
								case CHECKMATE:strcat(algebraic, "#");
							        break;
							}
						}
					...
					case XLALG: {
						...
						if ((output_format == ELALG || output_format == XLALG) && move_details->class == PIECE_MOVE) {
							strcpy(algebraic, piece_str(move_details->piece_to_move));
							ind = strlen(algebraic);
						}
						/* Format the basics. */
						if (move_details->class != NULL_MOVE) {
							sprintf(&algebraic[ind], "%c%c", move_details->from_col, move_details->from_rank);
							ind += 2;
							...
							sprintf(&algebraic[ind], "%c%c", move_details->to_col, move_details->to_rank);
							ind += 2;
						} else {
						...
						switch (move_details->class) {
							...
							case ENPASSANT_PAWN_MOVE:
						        ...
						        break;
							case PAWN_MOVE_WITH_PROMOTION: sprintf(&algebraic[ind],
							                                       "%s",
				        ...
				        break;
					default: fprintf(GlobalState.logfile, "Unknown output format %d in print_move()\n", output_format);
				        exit(1);
				        break;
				}
			} else {
				/* An empty move. */
				fprintf(GlobalState.logfile, "Internal error: Empty move in print_move.\n");
				report_details(GlobalState.logfile);
			}
			...
			if ((GlobalState.keep_variations) && (variants != NULL)) {
				while (variants != NULL) {
					 ...
					 */
					print_move_list(outputfile, move_number, white_to_move, variants->moves, (const Board *) NULL);
					single_char(outputfile, ')');
...
static void output_cm_comment(CommentList *comment, FILE *outputfile, unsigned indent) {   /* Don't indent for the first comment line, because
		...
		for (; comment_str != NULL; comment_str = comment_str->next) {
			...
			while (chunk != NULL) {
				...
				if ((line_length + 1 + len) > GlobalState.max_line_length) {
					...
					fprintf(outputfile, "\n%*s%c ", indent_for_this_line, "", CM_COMMENT_CHAR);
...
static void output_cm_game(FILE *outputfile, unsigned move_number, Boolean white_to_move, const Game game) {
	const Move *move = game.moves;

	if ((move_number != 1) || (!white_to_move)) {
		fprintf(GlobalState.logfile, "Unable to output CM games other than from the starting position.\n");
		report_details(GlobalState.logfile);
	}
		...
		if ((move->Comment != NULL) && GlobalState.keep_comments) {
			...
			if ((result != NULL) && (move->check_status != CHECKMATE)) {
				...
				if (white_to_move) {
					...
					fprintf(outputfile, "%*s %*s%*s", MOVE_NUMBER_WIDTH + 1, "", -MOVE_WIDTH, "...", MOVE_WIDTH, "");
				}
			...
			if ((move->terminating_result != NULL) && (move->check_status != CHECKMATE)) {
				/* Give some information on the nature of the finish. */
				const char *result = move->terminating_result;
...
void output_game(Game current_game, FILE *outputfile) {
	...
	if (final_board != NULL) {
		...
		switch (GlobalState.output_format) {
			...
			case HALG:
			case ELALG:
			case XLALG: print_algebraic_game(current_game, outputfile, move_number, white_to_move, final_board);
		        break;
			case EPD: print_epd_game(current_game, outputfile, move_number, white_to_move, final_board);
		        break;
			case CM: output_cm_game(outputfile, move_number, white_to_move, current_game);
		        break;
			default:
				fprintf(GlobalState.logfile,
...
void add_to_output_tag_order(TagName tag) {
	 ...
	 * for this tag.
	 */
	for (tag_index = 0; (TagOrder[tag_index] != -1) && (TagOrder[tag_index] != (int) tag); tag_index++) {
	}
	...
	if (TagOrder[tag_index] == -1) {
		...
		if (tag_index >= tag_order_space) {
			/* Allocate some more. */
			tag_order_space += 10;
			TagOrder = (int *) ReallocOrDie((void *) TagOrder, tag_order_space * sizeof(*TagOrder));
		}
...
static const char *format_epd_game_comment(char **Tags) {
	...
	if (strlen(comment) >= space_needed) {
		fprintf(GlobalState.logfile, "Internal error: overflow in format_epd_game_comment\n");
	}
	return comment;
}

static void print_algebraic_game(Game current_game,
                                 FILE *outputfile,
                                 unsigned move_number,
                                 Boolean white_to_move,
                                 Board *final_board) {
	...
	print_move_list(outputfile, move_number, white_to_move, current_game.moves, final_board);
	...
	if (current_game.moves == NULL) {
		if (current_game.tags[RESULT_TAG] != NULL) {
			...
			fprintf(GlobalState.logfile, "Internal error: Zero move game with no result\n");
		}
	}
	terminate_line(outputfile);
	putc('\n', outputfile);
...
static void print_epd_move_list(Game current_game,
                                FILE *outputfile,
                                unsigned move_number,
                                Boolean white_to_move,
                                Board *final_board) {
...
static void print_epd_game(Game current_game,
                           FILE *outputfile,
                           unsigned move_number,
                           Boolean white_to_move,
                           Board *final_board) {
...
static const char *build_FEN_comment(const Board *board) {
	...
	if (strlen(fen) >= FEN_SPACE) {
		fprintf(GlobalState.logfile, "Internal error: string overflow in build_FEN_comment.\n");
	}
	return fen;
}
