...
void search_check(search_input_t* SI, search_root_t* SR, search_current_t* SC, search_info_t* SIn) {

	search_send_stat();
	event();

	if (SI->depth_is_limited
	 && SR->depth > SI->depth_limit)
		SR->flag = true;


	if (SI->time_is_limited
	 && SC->time >= SI->time_limit_2)
		SR->flag = true;


	if (SI->time_is_limited
	 && SC->time >= SI->time_limit_1
	 && !SR->bad_1
	 && !SR->bad_2
	 && (!UseExtension || SR->move_pos == 0))
		SR->flag = true;


	if (SIn->can_stop
	&& (SIn->stop || (SR->flag && !SI->infinite)))
		longjmp(SIn->buf,1);
}
