...
uint_fast8_t ThreadNumber;
std::mutex trans_lock, search_lock;

search_param_t SearchStackMT[MaxThread][HeightMax];

search_input_t SearchInputMT[MaxThread][1];
search_info_t SearchInfoMT[MaxThread][1];
search_root_t SearchRootMT[MaxThread][1];
search_current_t SearchCurrentMT[MaxThread][1];
search_best_t SearchBestMT[MaxThread][MultiPVMax];
...
static int_fast32_t  full_search          (board_t * board, int_fast32_t alpha, int_fast32_t beta, int_fast32_t depth, int_fast32_t height, mv_t pv[], int_fast32_t node_type, uint_fast8_t thread_id);
...
static int_fast32_t  full_quiescence      (board_t * board, int_fast32_t alpha, int_fast32_t beta, int_fast32_t depth, int_fast32_t height, mv_t pv[], uint_fast8_t thread_id);
...
void search_full_init(list_t * list, board_t * board) {
    ...
    //threads
    ThreadNumber = option_get_int("Thread Number");
	// draw value
...
void sync_threads()  {
    for (uint_fast8_t th = 0 ; th < ThreadNumber; ++th)  {
        memcpy(SearchStackMT[th], SearchStack, HeightMax * sizeof(search_param_t));

        memcpy(SearchInputMT[th], SearchInput, sizeof(search_input_t));
        memcpy(SearchInfoMT[th], SearchInfo, sizeof(search_info_t));
        memcpy(SearchRootMT[th], SearchRoot, sizeof(search_root_t));
        memcpy(SearchCurrentMT[th], SearchCurrent, sizeof(search_current_t));
        memcpy(SearchBestMT[th], SearchBest, sizeof(search_best_t) * MultiPVMax);
        SearchCurrentMT[th]->node_nb = 0;
    }
}
// search_full_root()
...
static int_fast32_t full_root(list_t * list, board_t * board, int_fast32_t alpha, int_fast32_t beta, int_fast32_t depth, int_fast32_t height, int_fast32_t search_type) {
   ...
   best_value[SearchCurrent->multipv] = ValueNone;
	...
	for (int_fast32_t i = 0; i < LIST_SIZE(list); ++i) {
		...
		search_update_root();
		int_fast32_t new_depth = full_new_depth(depth,move,board,board_is_check(board)&&LIST_SIZE(list)==1,true, height);
		...
		if (search_type == SearchShort || best_value[SearchCurrent->multipv] == ValueNone) { // first move
            for (uint_fast8_t th = 0; th < ThreadNumber - 1; ++th)  {
                mv_t th_new_pv[HeightMax];
                board_t b2[1];
                board_deep_copy(b2, board);

                threads.push_back(new std::thread(full_search, b2,-beta,-alpha,new_depth+1,height+1,th_new_pv,NodePV, th));
                threads[th]->detach();
            }
			...
			if (value <= alpha)  { // research
                ...
                for (uint_fast8_t th = 0; th < ThreadNumber - 1; ++th)  {
                    ...
                    board_t b2[1];
                    board_deep_copy(b2, board);
                    threads.push_back(new std::thread(full_search, b2,-beta,ValueInf,new_depth+1,height+1,th_new_pv,NodePV,th));
                    threads[th]->detach();
                }
				value = -full_search(board,-beta,ValueInf,new_depth,height+1,new_pv,NodePV,0);
				for (uint_fast8_t th = 0; th < ThreadNumber - 1; ++th)  {
                    delete threads[th];
                }
                ...
                for (uint_fast8_t th = 0; th < ThreadNumber - 1; ++th)  {
                    ...
                    board_t b2[1];
                    board_deep_copy(b2, board);
                    threads.push_back(new std::thread(full_search, b2,-ValueInf,-alpha,new_depth+1,height+1,th_new_pv,NodePV,th));
                    threads[th]->detach();
                }
				value = -full_search(board,-ValueInf,-alpha,new_depth,height+1,new_pv,NodePV,0);
				for (uint_fast8_t th = 0; th < ThreadNumber - 1; ++th)  {
                    delete threads[th];
                }
			}
            ...
            for (uint_fast8_t th = 0; th < ThreadNumber - 1; ++th)  {
                ...
                board_t b2[1];
                board_deep_copy(b2, board);
                threads.push_back(new std::thread(full_search, b2,-alpha-1,-alpha,new_depth+1,height+1,th_new_pv,NodeCut,th));
                threads[th]->detach();
            }
		...
		for (uint_fast8_t th = 0; th < ThreadNumber; ++th)
            SearchCurrent->node_nb += SearchCurrentMT[th]->node_nb;
...
static int_fast32_t full_search(board_t * board, int_fast32_t alpha, int_fast32_t beta, int_fast32_t depth, int_fast32_t height, mv_t pv[], int_fast32_t node_type, uint_fast8_t thread_id) {
	...
	// horizon?
	if (depth <= 0) return full_quiescence(board,alpha,beta,0,height,pv, thread_id);

	// init
	search_lock.lock();
	SearchStackMT[thread_id][height].best_move = MoveNone;
	SearchStackMT[thread_id][height].move = MoveNone;
	SearchStackMT[thread_id][height].threat_move = MoveNone;
	SearchStackMT[thread_id][height].reduced = false;
   //bool mate_threat = false;

	SearchCurrentMT[thread_id]->node_nb++;
	SearchInfoMT[thread_id]->check_nb--;
	PV_CLEAR(pv);

	if (height > SearchCurrentMT[thread_id]->max_depth) SearchCurrentMT[thread_id]->max_depth = height;

	if (SearchInfoMT[thread_id]->check_nb <= 0) {
		SearchInfoMT[thread_id]->check_nb += SearchInfoMT[thread_id]->check_inc;
		search_check(SearchInputMT[thread_id], SearchRootMT[thread_id], SearchCurrentMT[thread_id], SearchInfoMT[thread_id]);
	}
    search_lock.unlock();
	// draw?
	...
	if (UseNull && depth >= nullDepth && node_type != NodePV) {
		if (!in_check
		 ...
		 && (!UseNullEval || depth <= NullReduction+1 || eval(board,alpha, beta, false, in_check) >= beta)) {
			...
			value = -full_search(board,-beta,-beta+1,new_depth,height+1,new_pv,NODE_OPP(node_type), thread_id);
			...
			if (UseVer && depth > VerReduction) {
				if (value >= beta && (!UseVerEndgame || do_ver(board))) {
					...
					if (value >= beta) {
						...
						best_move = move;
						search_lock.lock();
						SearchStackMT[thread_id][height].move = move;
						SearchStackMT[thread_id][height].best_move = move;
						search_lock.unlock();
						best_value = value;
						pv_copy(pv,new_pv);
						goto cut;
            ...
            search_lock.lock();
			SearchStackMT[thread_id][height].threat_move = SearchStack[height+1].best_move;
			search_lock.unlock();
...
/*	mate_threat = false;
	...
	if (depth >= IIDDepth && node_type == NodePV && trans_move == MoveNone) {
		...
		value = full_search(board,alpha,beta,new_depth,height,new_pv,node_type, thread_id);
		if (value <= alpha) value = full_search(board,-ValueInf,beta,new_depth,height,new_pv,node_type, thread_id);
	...
	while ((move=sort_next(sort)) != MoveNone) {
        ...
        search_lock.lock();
		SearchStackMT[thread_id][height].reduced = reduced;
		search_lock.unlock();
		...
		if (node_type != NodePV || best_value == ValueNone) { // first move
			value = -full_search(board,-beta,-alpha,new_depth,height+1,new_pv,NODE_OPP(node_type), thread_id);
		} else { // other moves
			value = -full_search(board,-alpha-1,-alpha,new_depth,height+1,new_pv,NodeCut, thread_id);
			if (value > alpha) { // && value < beta
				value = -full_search(board,-beta,-alpha,new_depth,height+1,new_pv,NodePV, thread_id);
			}
		}
...
static int_fast32_t full_no_null(board_t * board, int_fast32_t alpha, int_fast32_t beta, int_fast32_t depth, int_fast32_t height, mv_t pv[], int_fast32_t node_type, int_fast32_t trans_move, int_fast32_t * best_move) {
	...
	if (SearchInfo->check_nb <= 0) {
		...
		search_check(SearchInput, SearchRoot, SearchCurrent, SearchInfo);
	}
	...
	*best_move = MoveNone;
	...
	while ((move=sort_next(sort)) != MoveNone) {
		...
		move_do(board,move,undo);
		int_fast32_t value = -full_search(board,-beta,-alpha,new_depth,height+1,new_pv,NODE_OPP(node_type), 0);
		move_undo(board,move,undo);
...
static int_fast32_t full_quiescence(board_t * board, int_fast32_t alpha, int_fast32_t beta, int_fast32_t depth, int_fast32_t height, mv_t pv[], uint_fast8_t thread_id) {
	...
	if (SearchInfoMT[thread_id]->check_nb <= 0) {
		SearchInfoMT[thread_id]->check_nb += SearchInfo->check_inc;
		search_check(SearchInputMT[thread_id], SearchRootMT[thread_id], SearchCurrentMT[thread_id], SearchInfoMT[thread_id]);
	}
	search_lock.unlock();
